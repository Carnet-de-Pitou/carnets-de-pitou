...
function parseLibrary(source){const marker='window.PITOU_PUBLIC_LIBRARY = ';const i=source.indexOf(marker);if(i<0)throw new Error('Bibliothèque GitHub illisible');const start=i+marker.length;let raw=source.slice(start).trim();if(raw.endsWith(';'))raw=raw.slice(0,-1);const list=JSON.parse(raw);if(!Array.isArray(list))throw new Error('Bibliothèque GitHub invalide');return list}
